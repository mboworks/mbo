...
TEST_F(HamtLookupTest, BorrowsCallableStateAndRejectsHashesBeforeKeyExtraction) {
  ...
  constexpr auto kEntries = std::to_array<Entry>({Entry{.hash = 2, .key = 20}, Entry{.hash = 2, .key = 21}});
...
TEST_F(HamtLookupTest, TraversesBitmapNodesAndTerminalCollisions) {
  ...
  auto* const collision = Node::TryCreateCollision(source, kCollisions).value_or(nullptr);
  ...
  auto* const root = Node::TryCreate(source, root_index, kRootEntries, children).value_or(nullptr);
...
TEST_F(HamtLookupTest, TraversesTheEntireHashWidthForEverySupportedFragmentSize) {
  const auto check_width = []<std::size_t FragmentBits>() {
    ...
    auto* const leaf = DeepNode::TryCreate(source, leaf_index, kEntries, {}).value_or(nullptr);
    ...
    for (std::size_t level = Path::kLevels - 1; level > 0; --level) {
      ...
      auto* const parent = DeepNode::TryCreate(source, index, {}, children).value_or(nullptr);
...
TEST_F(HamtLookupTest, DirectEntryRequiresBothFullHashAndKeyToMatch) {
  ...
  auto* const node = Node::TryCreate(source, index, kEntries, {}).value_or(nullptr);
  ASSERT_THAT(node, NotNull());
