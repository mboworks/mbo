...
TEST_F(HamtNodeValueTest, EmptyHandlesRemainEmptyAcrossCopyMoveAndSwap) {
  Payload first;
...
TEST_F(HamtNodeValueTest, AssignmentsReleaseReplacedOwnershipAndPreserveSharedPayloads) {
  ...
  auto first_result = HeapPayload::TryCreate(domain, 42);
  ...
  second = first;
  EXPECT_THAT(second.get(), Eq(first_address));
  EXPECT_THAT(first.is_unique(), Eq(false));
  HeapPayload moved;
