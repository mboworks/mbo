...
The initial benchmark report and configuration matrix use the initial reference host:

- Apple M5 Pro, macOS arm64, Clang 22.

These results and production configuration recommendations are provisional.
AMD Ryzen 9 9950X (Zen 5), Linux x86-64, Clang 22, plus GCC 14 where code
generation differs, is later follow-up: it is not a gate for completing the initial
implementation sequence or its first benchmark report. Revisit recommendations
after comparing validated JSON from the same clean implementation on both hosts;
do not claim cross-architecture superiority from the initial host alone.
