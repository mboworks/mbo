...
struct QueryCountingIndex final {
  ...
  std::optional<bool> try_insert(std::string_view key, StringId<> identifier) noexcept {
    return index.try_insert(key, identifier);
  }
...
TEST_F(StringInternerTest, DeepChainsFilterEveryAncestorsLaterInsertionsInBothDirections) {
  ...
  for (std::size_t depth = 0; depth < kDepth; ++depth) {
    ...
    const StringId<> identifier(static_cast<std::uint32_t>(depth));
    EXPECT_THAT(leaf.find(name), Optional(identifier));
    EXPECT_THAT(leaf.rfind(name), Optional(identifier));
    EXPECT_THAT(leaf.get(identifier), Optional(std::string_view(name)));
  }
